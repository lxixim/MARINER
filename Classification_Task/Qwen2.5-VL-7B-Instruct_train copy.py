...
for i, arg in enumerate(sys.argv):
    if arg == "--cuda_visible_datasets" and i + 1 < len(sys.argv):
...
def parse_args():
    ...
    parser.add_argument("--lora_dropout", type=float, default=0.05,
                        help="LoRA dropout")
    parser.add_argument("--target_modules", type=str, nargs="+", default=LORA_TARGET_MODULES,
...
class CustomProgressCallback(TrainerCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        self.pbar = tqdm(total=state.max_steps, desc="Epoch Progress", leave=False)
        self.last_step = 0
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        if self.pbar and logs and "loss" in logs:
            self.pbar.set_postfix(loss=round(logs["loss"], 4))
