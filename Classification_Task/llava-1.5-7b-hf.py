...
def run():
    ...
    if not os.path.exists(mp): 
        mp = os.path.join(MODEL_ROOT, "llava-hf", MODEL_NAME)
    if not os.path.exists(mp):
        mp = "llava-hf/llava-1.5-7b-hf"

    print(f"正在加载模型：{mp}")
    
    # LLaVA-1.5 使用 AutoProcessor + LlavaForConditionalGeneration
    ...
    for name in tqdm(imgs, desc="推理中"):
        try:
            ...
            # 【关键】确保 pixel_values 与模型 dtype 一致 (bfloat16)
