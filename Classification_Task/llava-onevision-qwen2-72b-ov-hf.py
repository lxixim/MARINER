...
# ───────────────────────── 配置参数 ─────────────────────────
MODEL_NAME  = "llava-onevision-qwen2-72b-ov-hf"  # ✅ 改为 72B
MODEL_ROOT  = "/data/cn/llama/model"
DATA_ROOT   = "/data/cn/llama/classify_all/test/test"
OUTPUT_ROOT = "/data/cn/llama/Classify_Output"

# ✅ 取消单卡限制，让 accelerate / device_map 自动分配到多卡
# os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # ❌ 注释掉！

# ───────────────────────── 63 类船舶类型限定 ─────────────────────────
SHIP_CLASSES = [
    "054A", "Admiral_Gorshkov", "Arleighburke", "Asagiri", "Atago", "Austin",
    "Barge", "Bitumen", "Bluerridge", "Bulk_carrier", "Cavour", "Charles_de_Gaulle",
...
def run():
    ...
    if not os.path.exists(mp):
        mp = "lmms-lab/llava-onevision-qwen2-72b-ov-hf"  # ✅ HuggingFace 官方路径

    print(f"正在加载 72B 模型：{mp}")
    
    # 加载 processor
    processor = AutoProcessor.from_pretrained(mp, trust_remote_code=True)
    
    # ✅ 关键：使用 device_map="auto" 自动分配到多 GPU
    model = AutoModelForVision2Seq.from_pretrained(
        mp,
        torch_dtype=torch.bfloat16,      # ✅ 72B 必须用 bfloat16（若硬件支持）
        low_cpu_mem_usage=True,
        trust_remote_code=True,
        device_map="auto"                # ✅ 自动分配到可用 GPU（如 0,1）
    ).eval()
    ...
    for name in tqdm(imgs, desc="推理中"):
        try:
            ...
            # ✅ 将 inputs 移动到模型所在设备（自动处理多卡）
            inputs = {k: v.to(model.device) for k, v in inputs.items()}
            
            # 【关键】确保 pixel_values 与模型 dtype 一致
