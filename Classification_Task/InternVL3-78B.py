import os, json, torch, warnings, re, gc  # 添加gc导入
...
os.environ["CUDA_VISIBLE_DEVICES"] = "0,1"  # 使用双卡
...
def get_custom_device_map():
    ...
    for i in range(80):
        ...
        elif 33 <= i <= 78:  # 中间46层在GPU 1
            device_map[f'language_model.model.layers.{i}'] = 1
        elif i == 79:  # 最后1层回GPU 0
            device_map[f'language_model.model.layers.{i}'] = 0
...
def run():
    ...
    try:
        print("尝试以 INT8 (8-bit) 加载模型...")
        device_map = get_custom_device_map()
        ...
        print("✅ 成功以 INT8 加载模型 (双卡分配)")
    except Exception as e1:
        print(f"❌ INT8 加载失败：{e1}")
        print("回退到 bfloat16 + 自动分配...")
        try:
            model = AutoModel.from_pretrained(
                mp,
            ...
            raise RuntimeError(f"模型加载完全失败：INT8 错误={e1}, 回退错误={e2}")

    # 验证双卡使用情况
    if torch.cuda.device_count() > 1:
        print(f"✅ 检测到 {torch.cuda.device_count()} 个GPU，模型已分配到双卡")
    else:
        print("⚠️ 仅检测到1个GPU，可能未正确配置双卡环境")
    ...
    for name in tqdm(imgs, desc="推理中"):
        try:
            ...
            # 确保输入张量在GPU 0（与模型输入层一致）
            pv = pv.to('cuda:0').to(torch.float16)
    ...
    print(f"统计：具体类别 {stats['specific']}，Others {stats['others']}，错误 {stats['error']}")
    print(f"匹配方式：{match_stats}")
